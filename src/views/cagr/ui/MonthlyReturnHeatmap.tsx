...
const CELL_CLASS_NAME = "h-[26px] min-w-10 px-1 py-0 text-center";

/** 스크롤되는 셀을 가려야 하므로 반드시 불투명색임. 페이지 배경과 살짝 달라 축 구실도 함. */
const YEAR_HEADER_CLASS_NAME =
  "sticky left-0 z-10 h-[26px] bg-neutral-100 px-1 py-0 text-center font-semibold dark:bg-neutral-900";

const ANNUAL_COLUMN_CLASS_NAME =
  "sticky right-0 z-10 border-l border-neutral-300 bg-neutral-100 pl-1 shadow-[-4px_0_6px_-6px_rgba(0,0,0,0.35)] dark:border-neutral-600 dark:bg-neutral-900";
...
const MonthlyReturnHeatmap = ({ rows }: MonthlyReturnHeatmapProps) => {
  ...
  const HeadRowTemplate = (
    <tr>
      ...
      {MONTH_COLUMN_LABELS.map((monthLabel) => (
        <th key={monthLabel} scope="col" className="h-[26px] px-1 py-0 font-semibold">
          {monthLabel}
        </th>
      ))}
      <th scope="col" className={`h-[26px] px-1 py-0 font-bold ${ANNUAL_COLUMN_CLASS_NAME}`}>
        연간
      </th>
    </tr>
  );
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      */}
      <div className="cagr-table-scrollbar -mx-2 overflow-x-auto pb-2">
        <table className="w-full border-separate border-spacing-[1px] font-number text-[13px] leading-[18px] tabular-nums">
