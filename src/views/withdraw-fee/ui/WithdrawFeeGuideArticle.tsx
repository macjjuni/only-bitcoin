import { GuideArticle, GuideParagraph, GuideSection } from "@/shared/ui";

/** 검색 유입을 위한 설명 섹션. 접힌 내용도 HTML에 포함되는 네이티브 details를 사용한다. */
export default function WithdrawFeeGuideArticle() {
  return (
    <GuideArticle
      title="출금 수수료, 왜 이렇게 비싼가요?"
      subtitle="거래소 출금 수수료가 어떻게 정해지는지, 비트코인을 개인 지갑으로 옮길 때 무엇을 확인해야 하는지 정리했습니다."
    ...
    >
      ...
      <GuideSection title="왜 고정 금액으로 받나요?">
        <GuideParagraph>
          ...
          드는 비용도 있습니다. 다만 거래소 출금 수수료는 네트워크가 한산해져도 즉시 낮아지지 않을
          수 있습니다.
        </GuideParagraph>
      </GuideSection>
      ...
      <GuideSection title="자산·네트워크마다 출금 수수료가 다른 이유는 무엇인가요?">
        <GuideParagraph>
          거래소 출금 수수료는 자산과 네트워크별 정책으로 정해지며, 해당 네트워크의 실제 전송 비용과
          일치하지 않을 수 있습니다. 수수료가 싸다는 이유로 다른 자산이나 네트워크를 선택하면
          비트코인을 출금하는 것이 아닙니다. 비트코인을 자기수탁하려면 수신 지갑이 지원하는 Bitcoin
          네트워크를 선택해야 합니다.
        </GuideParagraph>
      </GuideSection>
      ...
      <GuideSection title="출금 수수료를 아끼려면 어떻게 하나요?">
        ...
        <GuideParagraph>
          국내 거래소보다 비트코인 출금 수수료가 낮은 해외 거래소를 이용하는 것도 방법입니다. 다만
          거래소의 홍보나 평판을 그대로 믿지 말고, 실제 출금 수수료와 최소 출금액, 출금 가능 여부를
          직접 확인하세요. 처음 이용할 때는 소액을 개인 지갑으로 출금해 검증하고, 필요한 거래가
          끝나면 거래소에 머무는 금액과 시간을 줄이는 편이 좋습니다. 낮은 수수료를 선택하며 생기는
          보관·출금 위험은 각자가 감당해야 합니다.
        </GuideParagraph>
      </GuideSection>

      <GuideSection title="그럼에도 왜 출금해야 하나요?">
        <GuideParagraph>
          거래소 계정의 잔고는 거래소가 나에게 갚아야 할 빚이지, 내가 가진 비트코인이 아닙니다. 개인
          키를 내가 들고 있어야 비로소 내 비트코인입니다. 출금 수수료는 그 소유권을 확보하는
